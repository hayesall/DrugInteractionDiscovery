...
__[Alexander L. Hayes](http://batflyer.net) | [Savannah Smith](https://savannahkimbree.wordpress.com/) | Devendra Dhami | Sriraam Natarajan__
...
Questions?  Contact Alexander Hayes at __hayesall(at)indiana(dot)edu__ or Savannah Smith at __savannah.smith(at)valpo(dot)edu__
...
This folder contains the final deliverables for the ProHealth Summer REU 2016.
  ...
  * [Presentation Poster (png/pdf/psd)](Overview/Poster)
  * [Project Overview Video - YouTube](https://www.youtube.com/watch?v=yshy8NyhkZk)
...
1. `builddruglist.sh` [View Code](openFDA/builddruglist.sh)
...
2. `rxdownloader.sh` [View Code](openFDA/rxdownloader.sh)
...
3. `fixlist.sh` [View Code](openFDA/fixlist.sh)
...
2. `smartsplit.sh` [View Code](PubMed/smartsplit.sh)
...
3. `pullabstracts.sh` [View Code](PubMed/pullabstracts.sh)
...
4. `pullabstractsODIN.sh` [View Code](PubMed/pullabstractsODIN.sh)
...
[Return to Top](#drug-interaction-discovery-with-nlp-and-machine-learning) | [View in Folder](PubMed/)

---

#####SocialBlogs:

Here you will find several attempts to automatically extract text from [DailyStrength.org](https://www.dailystrength.org).

In order to run [seventh_attempt.py](SocialBlogExtract/seventh_attempt.py) you will need to download/install ['Beautiful Soup.'](https://www.crummy.com/software/BeautifulSoup/)  Also make sure you include all necessary packages (refer to import statements in the code).  Until DailyStrength updated their website, this code ran as expected.  It extracted all social blog discussions from an instructed location.  As a result of the update, the links and tags were changed a bit, but this was an easy fix in Beautiful Soup.  The problem was a 'show more' button replacing the option to go to the next page.  To solve this, I needed to write a script that could interact with the web page (i.e. push the 'show more' button).

The best option to solve this (after some brief research) appeared to be the ['Selenium WebDriver.'](http://www.seleniumhq.org/projects/webdriver))  I wrote a script in python using the Chrome WebDriver.  This WebDriver can open a browser, search in a browser, and interact with it in other ways.  This code was weird in the way that it would crash unexpectedly after running correctly on a similar task.  It seemed that [DailyDraft.py](SocialBlogExtract/DailyDraft.py) needed multiple 'scroll element into view' and multiple 'scroll up to the top of the page' commands.  Running [FinalDaily.py](SocialBlogExtract/FinalDaily.py) will take some time.
...
My final code required a lot of `time.sleep()`'s to pause the code and `driver.implicitly_wait()`'s to pause and give the webpage time to load before completing the next step.  I have two output files from FinalDaily.py linked to this repository:
  ...
  * [DailyStrength1.txt](SocialBlogExtract/DailyStrength1.txt) has the discussion text of links 1-98 under the result of searching "Drug Interactions."
  * [DailyStrength2.txt](SocialBlogExtract/DailyStrength2.txt) has the output of links 99-130.

[Return to Top](#drug-interaction-discovery-with-nlp-and-machine-learning) | [View in Folder](SocialBlogExtract/)
