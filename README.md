...
The best option to solve this (after some brief research) appeared to be the ['Selenium WebDriver.'](www.seleniumhq.org/projects/webdriver))  I wrote a script in python using the Chrome WebDriver.  This WebDriver can open a browser, search in a browser, and interact with it in other ways.  This code was weird in the way that it would crash unexpectedlyafter running correctly on a similar task.  It seemed that [DailyDraft.py](SocialBlogExtract/DailyDraft.py) needed multiple 'scroll element into view' and multiple 'scroll up to the top of the page' commands.  Running [FinalDaily.py](SocialBlogExtract/FinalDaily.py) will take some time.
